...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        :return:
        """
        return ["[PAD]", "-", "PER_B", "PER_I", "FLD_B", "FLD_I", "AFW_B", "AFW_I", "ORG_B", "ORG_I", 
                "LOC_B", "LOC_I", "CVL_B", "CVL_I", "DAT_B", "DAT_I", "TIM_B", "TIM_I", "NUM_B", "NUM_I", 
                "EVT_B", "EVT_I", "ANM_B", "ANM_I", "PLT_B", "PLT_I", "MAT_B", "MAT_I", "TRM_B", "TRM_I", 
                "X","[CLS]","[SEP]"]
    ...
    with open(output_predict_file,'w') as wf:
        if  FLAGS.crf:
            ...
            for m,pred in enumerate(result):
                ...
                # call method for recover_tokens
                predictions = recover_pred(predictions)
...
### added by hyojin @2019-09-19
def recover_tokens(batch_tokens, batch_labels):
    ## batch_tokens 처리
    # 어절 단위로 변형하기 위해 tokens를 하나로 합침
    batch_tokens = ''.join(batch_tokens)

    # batch_tokens에 있는 token 형태의 값에 '_' 추가
    batch_tokens = batch_tokens.replace('[CLS]', '[CLS]_')
    batch_tokens = batch_tokens.replace('[PAD]', '[PAD]_')

    # '_'로 split하여 어절 단위로 끊어 리스트 형태로 저장
    batch_tokens = batch_tokens.split('_')

    # '_' split 과정 중 남은 dump 데이터 처리
    for token in batch_tokens:
        if token == '':
            batch_tokens.remove('')

    # max_seq_length의 길이 만큼 batch_tokens에 '[PAD]' 추가
    tokens_length = len(batch_tokens)
    for i in range(tokens_length, FLAGS.max_seq_length):
        batch_tokens.append('[PAD]')
    ...
    for label in batch_labels:
        if label == 30:
            ...
            batch_labels_edit.append(label)

    # max_seq_length의 길이 만큼 batch_labels에 0 추가
    labels_length =len(batch_labels_edit)
    for i in range(labels_length, FLAGS.max_seq_length):
        batch_labels_edit.append(0)

    batch_labels = batch_labels_edit

    return batch_tokens, batch_labels

def recover_pred(predictions):
    ## predictions 처리
    # 'X'로 label된 정보 삭제
    predictions_edit = []
    for pred in predictions:
        if pred == 30:
            ...
            predictions_edit.append(pred)

    # max_seq_length의 길이 만큼 predictions에 0 추가
    predictions_length =len(predictions_edit)
    for i in range(predictions_length, FLAGS.max_seq_length):
        predictions_edit.append(0)

    predictions = predictions_edit
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        # call method for recover_tokens
        batch_tokens, batch_labels = recover_tokens(batch_tokens, batch_labels)
